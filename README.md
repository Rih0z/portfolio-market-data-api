...
全てのテストを実行するには次のスクリプトを使用します。

```bash
./scripts/run-tests.sh all
```

このコマンドはユニットテスト、統合テスト、E2Eテストをまとめて実行し、詳細なカバレッジレポートを生成します。


テストを実行する前に、`npm install` を実行して依存パッケージをインストールしておく必要があります。ネットワークに接続できない環境では、事前に `node_modules` ディレクトリを準備してください。
